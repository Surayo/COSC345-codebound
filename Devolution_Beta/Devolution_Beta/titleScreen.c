...
void init_title_screen(GameState *game){
    ...
    //Subtitle text:
    SDL_Surface *tmp4 = TTF_RenderText_Blended(game->subtitleFont, "A game filled with bugs >_<", white);
    ...
    //Footer text:
    SDL_Surface *tmp5 = TTF_RenderText_Blended(game->footerFont, "CodeBound 2019", white);
    game->footerW = tmp5->w;
    game->footerH = tmp5->h;
    game->footer = SDL_CreateTextureFromSurface(game->renderer, tmp5);
    SDL_FreeSurface(tmp5);
    //Instructions text:
    SDL_Surface *tmp6 = TTF_RenderText_Blended_Wrapped(game->footerFont, "Use the arrow keys to move selector Press spacebar to select an option", white, 600);
    game->instructionsW = tmp6->w;
    game->instructionsH = tmp6->h;
    game->instructions = SDL_CreateTextureFromSurface(game->renderer, tmp6);
...
void draw_title_screen(GameState *game){
    ...
    // draw the SUBTITLE //
    SDL_Rect subtitleRect = { game->screenCenterX-game->subtitleW/2, game->screenCenterY-120, game->subtitleW, game->subtitleH};
